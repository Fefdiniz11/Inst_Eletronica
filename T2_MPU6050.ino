...
void eixoX(){
  ...
  for(i = 1; i <= 100; i++){
    ...
    acelerometro_X = dados(0x68, 0x3B);     // Le os valores do acelerometro no eixo X
...
void eixoY(){
  ...
  for(i = 1; i <= 100; i++){
    ...
    acelerometro_Y = dados(0x68, 0x3D);     // Le os valores do acelerometro no eixo Y
    grav_Y = ConversaoAcelerometro(acelerometro_Y);       // Converter para forca gravitacional  
    theta = (atan((-grav_Y/(sqrt ((grav_X*grav_X)) + ((grav_Z) *(grav_Z))) )))*180/M_PI;  // Calcular o angulo
    if(theta < 0){
      theta_Y = theta + 360;
    }else{
      theta_Y = theta;
...
void eixoZ(){
  ...
  for(i = 1; i <= 100; i++){
    ...
    acelerometro_Z = dados(0x68, 0x3F);     // Le os valores do acelerometro no eixo Z
